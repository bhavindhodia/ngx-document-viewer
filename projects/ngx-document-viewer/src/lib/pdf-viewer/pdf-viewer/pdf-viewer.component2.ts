...
import { createEventBus } from './utils/event-bus.utils2';
  ...
  implements OnChanges, OnInit, OnDestroy, AfterViewChecked
{
    ...
    from(this._pdf!.getPage(this.pdfViewer.currentPageNumber))
      ...
      .subscribe({
        next: (page: PDFPageProxy) => {
          ...
          if (stickToPage)
            console.log('stickToPage', this.pdfViewer._pages);
  ...
  private loadPDF() {
    ...
    console.log("CANVAS",this.pdfViewerContainer.nativeElement);
  ...
  private render() {
    ...
    if (!this.pdfViewer._pages?.length) {
      ...
      const sub = this.pageInitialized.subscribe(() => {
        console.log('pageInitialized',sub);
